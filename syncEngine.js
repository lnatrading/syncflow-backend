...
async function runSupplierSync(supabase, supplier) {
  ...
  try {
    ...
    if (categoriesEndpoint) {
      const catUrl = versionResolver.resolveEndpointUrl(categoriesEndpoint, activeVersion);
      const rawCats = await fetchEndpoint(catUrl, categoriesEndpoint.format, auth).catch(() => []);
      discoverCategories(supabase, supplier.id, rawCats).catch(e =>
        console.warn('[SYNC] Category discovery failed:', e.message)
      );
    }
    ...
    if (stockAlerts.length) {
      // Insert stock alerts in batches to avoid oversized payloads
...
function mergeEndpointData(products, secondaryData, role) {
  ...
  // Build lookup by SKU (try common field names)
  const byId = {};
  for (const item of secondaryData) {
    const key = item.sku || item.ref || item.code || item.id || item.productId;
    if (key != null) byId[String(key)] = item;
  }

  return products.map(p => {
    const key = p.sku || p.ref || p.code || p.id;
...
function normaliseProduct(raw, mappings, markupRules) {
  ...
  if (!product.name)       product.name       = raw.name || raw.title || raw.product_name || 'Unknown';
  if (!product.cost_price) product.cost_price = parseFloat(raw.cost || raw.cost_price || raw.wholesale_price || 0);
  if (!product.sale_price) product.sale_price = parseFloat(raw.price || raw.sale_price || raw.list_price || 0);
  if (!product.stock_qty)  product.stock_qty  = parseInt(raw.qty || raw.stock || raw.quantity || raw.qty_available || raw.availableQty || 0, 10);
  if (!product.image_url)  product.image_url  = raw.image || raw.image_url || raw.img || raw.imageUrl || null;
  if (!product.category)   product.category   = raw._resolvedCategory || raw.category || raw.categ || raw.categoryName || null;
  if (!product.description) product.description = raw._descriptions || raw.description || raw.desc || null;
...
async function discoverAttributes(supabase, supplierId, rawProducts) {
  ...
  // Count non-empty occurrences of each top-level scalar field
  const counts = {};
  ...
  // Upsert all discovered attributes in one query
  ...
  // Use upsert with onConflict so re-running just updates the count
...
// Handles both flat arrays and nested tree structures.
// Builds the full path string by walking parent references if present.
async function discoverCategories(supabase, supplierId, rawCategories) {
  if (!rawCategories.length) return;

  // Build a flat list of { external_id, path, name } from whatever shape we get
  const flat = [];

  function extractCat(item, parentPath = '') {
    // Try common field names for id, name, children
    const id       = item.id       || item.categoryId || item.cat_id   || item.code   || null;
    const name     = item.name     || item.title      || item.label    || String(id)  || 'Unknown';
    const children = item.children || item.subcategories || item.subCategories || [];
    const path     = parentPath ? `${parentPath} > ${name}` : name;

    flat.push({ external_id: id ? String(id) : null, path, name });

    if (Array.isArray(children)) {
      for (const child of children) extractCat(child, path);
    }
  }
  ...
  const rows = flat.map(c => ({
    ...
    path:         c.path,
    name:         c.name,
    product_count: 0, // will be updated by a separate count pass below
  }));
