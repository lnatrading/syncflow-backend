      ...
      try {
        ...
        if (existingCats) {
          for (const cat of existingCats) {
            // Match category name or external_id to product ClassCode
            const count = catCounts[cat.name] || catCounts[cat.external_id] || 0;
...
function normaliseProduct(raw, mappings, markupRules) {
  ...
  // Extract image URLs from datasheets (TD Baltic IceCat format)
  // ProductPicture is an object: { "#text": "https://...", "@_descr": "Product Picture URL 1" }
  if (!product.image_url && raw._datasheetsData) {
    const ds = raw._datasheetsData;
    const pic = ds['ProductPicture'] || ds['ProductPicture1'];
    if (pic) {
      product.image_url = typeof pic === 'string' ? pic
        : pic['#text'] || pic['@_descr'] || null;
    }
  }
  // Extract long/short description from datasheets
  if (!product.description && raw._datasheetsData) {
    const ds = raw._datasheetsData;
    product.description = (typeof ds.LongDesc === 'string' && ds.LongDesc.trim())
      ? ds.LongDesc.trim()
      : (typeof ds.ShortDesc === 'string' && ds.ShortDesc.trim())
      ? ds.ShortDesc.trim() : null;
  }
