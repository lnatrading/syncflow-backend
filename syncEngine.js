...
function normaliseProduct(raw, mappings, markupRules) {
  ...
  // Extract image URLs and description from datasheets (TD Baltic IceCat format)
  if (raw._datasheetsData) {
    const ds = raw._datasheetsData;

    // Collect all ProductPicture URLs (up to 6)
    const imgUrls = [];
    for (const key of ['ProductPicture','ProductPicture1','ProductPicture2','ProductPicture3','ProductPicture4','ProductPicture5']) {
      const pic = ds[key];
      if (!pic) continue;
      const url = typeof pic === 'string' ? pic : (pic['#text'] || null);
      if (url && url.startsWith('http')) imgUrls.push(url);
    }
    ...
    // Highlights as supplementary description
    if (!product.highlights && ds.ProductHighLights) {
      const h = ds.ProductHighLights;
      product.highlights = typeof h === 'string' ? h : (h['#text'] || null);
    }
  }
