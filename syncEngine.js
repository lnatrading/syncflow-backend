...
async function runSupplierSync(supabase, supplier) {
  ...
  try {
    ...
    // Match products to categories by ClassCode = category name
      ...
      try {
        ...
        for (const [catName, count] of Object.entries(catCounts)) {
          await supabase.from('supplier_categories')
            ...
            .eq('supplier_id', supplier.id)
            .or(`name.eq.${catName},external_id.eq.${catName}`);
        }
...
function normaliseProduct(raw, mappings, markupRules) {
  ...
  if (!product.image_url && raw._datasheetsData) {
    ...
    // ProductPicture elements: try @_descr or direct URL fields
    const pic = ds['ProductPicture'] || ds['ProductPicture1'] || ds['@_ProductPicture'];
    if (pic) {
      product.image_url = typeof pic === 'string' ? pic
        : pic['@_descr'] || pic.descr || null;
    }
  }
